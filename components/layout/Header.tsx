...
export function Header() {
  ...
  const getPageTitle = () => {
    ...
    if (pathname.startsWith("/game")) return "GIWA: Last Stand";
    ...
    <header className="sticky top-0 z-10 bg-dark-navy/80 backdrop-blur-md border-b border-white/10 px-4 lg:px-8 py-4 flex items-center justify-between">
      <div className="flex items-center gap-3 lg:gap-6 flex-1 min-w-0">
        ...
        <div className="hidden xl:flex gap-4">
          <a
            className="text-sm text-muted-blue hover:text-[#2ee6a6] transition-colors"
      ...
      <div className="flex items-center gap-2 lg:gap-4">
        {isConnected && balance !== undefined && (
          <div className="px-2 lg:px-4 py-2 bg-[#2ee6a6]/30 border border-[#2ee6a6]/20 rounded-full flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-[#2ee6a6] animate-pulse"></div>
